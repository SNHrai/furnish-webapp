...
export default function CalculatorPage() {
  ...
  // Handle PDF download using Spring Boot service
  const handleDownloadPDF = async () => {
    try {
      const { formData: quotationFormData, calculation } = generateQuotationData()
      const { quotationServiceClient } = await import('@/lib/services/api-client')
      
      // Create a temporary quotation object
      const tempQuotation = {
        id: `temp-${Date.now()}`,
      ...
      // Generate PDF using Spring Boot service
      const pdfBlob = await quotationServiceClient.generateQuotationPDF(
        tempQuotation,
        {
          name: user?.full_name || 'Customer',
          email: user?.email || 'customer@example.com',
      ...
      if (pdfBlob) {
        ...
        link.href = url
        link.download = `quotation-${tempQuotation.id}.pdf`
        link.click()
        URL.revokeObjectURL(url)
        
        toast.success('PDF downloaded successfully!')
      } else {
        // Fallback to client-side PDF generation notification
        toast.error('PDF service unavailable. Please save quotation and download later.')
      }
    } catch (error) {
